...
class TestDatabaseModule:
    ...
    def test_get_database_url_missing_returns_empty(self, monkeypatch):
        monkeypatch.delenv("DATABASE_URL", raising=False)
        assert db_module.get_database_url() == ""

    def test_get_engine_raises_when_database_url_missing(self, monkeypatch):
        monkeypatch.delenv("DATABASE_URL", raising=False)
        with pytest.raises(ValueError, match="DATABASE_URL environment variable is not set"):
            db_module.get_engine()

    def test_get_engine_uses_env_database_url(self, monkeypatch):
    ...
    def test_get_db_initializes_sessionlocal_lazily(self, monkeypatch):
        monkeypatch.setenv("DATABASE_URL", "sqlite:///:memory:")
    ...
    def test_get_db_reuses_existing_sessionlocal(self, monkeypatch):
        monkeypatch.setenv("DATABASE_URL", "sqlite:///:memory:")
